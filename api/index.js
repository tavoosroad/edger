// =============================================================
//  High-Performance Edge Tunnel Relay
//  Optimized for Vercel Pro Core Architecture
// =============================================================
import { PassThrough, Readable, Transform } from "node:stream";
import { pipeline } from "node:stream/promises";
import { setDefaultResultOrder } from "node:dns";

const __TUNNEL_BUILD_ID__ = "edge-relay-core-optimized";
void __TUNNEL_BUILD_ID__;
...
// هماهنگی کامل زمان با لایه پرو ورسل
const UPSTREAM_TIMEOUT_MS = parsePositiveInt(process.env.UPSTREAM_TIMEOUT_MS, 280000, 1000);
const MAX_INFLIGHT = parsePositiveInt(process.env.MAX_INFLIGHT, 1024, 1); 

// حذف کامل محدودیت پهنای باند برای دسترسی به حداکثر سرعت ترافیک ورسل
const MAX_UP_BPS = 0; 
const MAX_DOWN_BPS = 0; 
...
export default async function handler(req, res) {
  ...
  if (!TARGET_BASE || !RELAY_PATH || RELAY_PATH === "/" || !PUBLIC_RELAY_PATH || PUBLIC_RELAY_PATH === "/") {
    res.statusCode = 500;
    return res.end("System Misconfigured: Core variables are missing.");
  }
  ...
  try {
    ...
    try {
      ...
      if (hasBody) {
        uploadNodeStream = GLOBAL_UPLOAD_LIMITER ? req.pipe(createThrottleTransform(GLOBAL_UPLOAD_LIMITER)) : req;
      ...
      for (const [headerName, headerValue] of upstream.headers) {
        ...
        try { res.setHeader(headerName, headerValue); } catch {}
      }
      ...
      if (!upstream.body) {
        ...
        const downloadStream = GLOBAL_DOWNLOAD_LIMITER ? upstreamNode.pipe(createThrottleTransform(GLOBAL_DOWNLOAD_LIMITER)) : upstreamNode;
...
function applyDnsPreference() {
  ...
  try { setDefaultResultOrder(UPSTREAM_DNS_ORDER); } catch {}
}

function isUpstreamTimeoutError(err) {
  if (!err) return false;
  return err?.name === "AbortError" || err?.message === "upstream_timeout" || err?.cause?.message === "upstream_timeout" || err === "upstream_timeout";
}
  ...
  return {
    acquire(maxBytes) {
      return new Promise((resolve) => {
        queue.push({ maxBytes: Math.max(1, Math.trunc(maxBytes || 1)), resolve });
        tryDrain();
        if (queue.length > 0 && !timer) {
          timer = setTimeout(() => { timer = null; tryDrain(); }, 5);
        }
      });
    },
  };
